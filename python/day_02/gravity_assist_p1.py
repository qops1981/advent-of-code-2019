...
def main():
    ...
    module_masses[1] = 12   # Per Challenge this value is set
    module_masses[2] = 2    # Per Challenge this value is set

    for i, compute_value in enumerate(module_masses):
        if i % 4 == 0:      # Shifting by 4 for Operator Value

            if compute_value == 99: # The 99 Operator Value signals HAULT
                break

            action, value_one_pos, value_two_pos, store_pos, *_ = module_masses[i:-1]   # Set Required Positions

            if  compute_value == 1:     # Operator 1 is addition of the provided positions
                module_masses[store_pos] = (module_masses[value_one_pos] + module_masses[value_two_pos])
